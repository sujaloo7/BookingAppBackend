...
import { getContactUs, updateContactUs } from "repositories/settingRepository"
...
const ContactUs = props => {
  ...
  const GetContactUs = async () => {
    ...
    setContactId(res.data._id)
  }

  async function handleValidSubmit(event, values) {
    let res = await updateContactUs({
      contact_us: contact,
      contactus_id: contactId,
    })
    if (res.status == 1) {
          ...
          >
            <div className="form-group">
              <CKEditor
                ...
                data={contact}
                config={{ placeholder: "enter contact us ..." }}
